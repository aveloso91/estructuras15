...
public class LinkedBinarySearchTree<E> implements BinarySearchTree<E> {
    ...
    public Iterable<Position<E>> findRange(E minValue, E maxValue) {
        ...
        findRangeAux(minValue,maxValue,this.binTree.root(),l);
        return l;
    }

    private void findRangeAux(E minValue,E maxValue,Position<E> actual,ArrayList<Position<E>> l){
        int compd = this.comparator.compare(actual.getElement(), maxValue);
        ...
        if(this.binTree.hasLeft(actual))
            findRangeAux(minValue,maxValue,this.binTree.left(actual),l);
        if(this.binTree.hasRight(actual))
            findRangeAux(minValue,maxValue,this.binTree.right(actual),l);
    ...
    public Iterable<Position<E>> successors(Position<E> pos) {
        ArrayList<Position<E>> elements = new ArrayList<Position<E>> ();
        if (this.binTree.hasRight(pos))
            route(elements, this.binTree.right(pos));
        while (!this.binTree.isRoot(pos)) {
            if (this.binTree.left(this.binTree.parent(pos))==pos) {
                elements.add(this.binTree.parent(pos));
                route(elements, this.binTree.right(this.binTree.parent(pos)));
            }
    ...
    protected void route(ArrayList<Position<E>> elementos, Position<E> p) {
        elementos.add (p);
        if (this.binTree.hasLeft(p))
            route(elementos, this.binTree.left(p));
        if (this.binTree.hasRight(p))
            route(elementos, this.binTree.right(p));
    ...
    public Iterable<Position<E>> predecessors(Position<E> pos) {
        ...
        if (this.binTree.hasLeft(pos))
            route(elements, this.binTree.left(pos));
        while (!this.binTree.isRoot(pos)) {
            if (this.binTree.right(this.binTree.parent(pos))==pos) {
                elements.add(this.binTree.parent(pos));
                route(elements, this.binTree.left(this.binTree.parent(pos)));
            }
            pos = this.binTree.parent (pos);
        }
        return elements;
